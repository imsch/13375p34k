...
public class Program {
	...
	// der Übersicht
	private static String alph = "Rindersalat";
	...
	// auszugeben
	// ("er sagte: \"Hallo!\"")
	private static String[][] alphabete = new String[33][2];
	...
	public static void uebersetzenLatZuLeet() {
		alph = alph.toUpperCase(); // Programm setzt Großbuchstaben voraus
		...
		for (int i = 0; i < alph.length(); i++) {
			for (int x = 0; x < alphabete.length; x++) {
				if (alphabete[x][1].contains(String.valueOf(alph.charAt(i)))) {
					// Es wird geprüft welcher Buchstabe an der Reihe ist. Der
					// entsprechnde Leet_Ausdruck wird in das Übersetzungsarray
					// gespeichert.
	...
	public static void uebersetzungAusgeben(String[] uebersetzung) {
		for (int x = 0; x < uebersetzung.length; x++) {
			System.out.print(uebersetzung[x] + " "); // Die einzelnen Buchstaben
														// werden durch
	...
	public static void dezDual(int dezimalZahl) // Umrechnung vom Dezimal- ins
											// Dualsystem
	{
		ArrayList<Integer> dual = new ArrayList<Integer>();
		do {
			dual.add(dezimalZahl % 2);
			dezimalZahl = dezimalZahl / 2;
		} while (dezimalZahl > 0);
		for (int i = dual.size() - 1; i >= 0; i--) {
			System.out.print(dual.get(i));
			
		}
		System.out.println();
	}
	
	public static void dualDez(String dualZahl){
		int erg=0;
		int multi=1;
		for(int i = dualZahl.length()-1;i>=0;i--)
		{
			String aktuelleZahl=""+dualZahl.charAt(i);
			int x=Integer.parseInt(aktuelleZahl);
			erg=erg+(x*multi);
			multi=multi*2;
